...
def update(frame):
    global data_points, ax, last_received_value, is_paused
    if not is_paused:
...
default_interval = 0.001  # Microseconds
...
update_interval = float(input(f"Enter the update interval in microseconds (default: {default_interval}): ") or default_interval)
...
# Create animation with the specified update interval
ani = FuncAnimation(fig, update, frames=None, interval=update_interval, blit=False)
...
button_ax = plt.axes([0.82, 0.02, 0.1, 0.04])
