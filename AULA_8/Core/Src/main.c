...
int main(void) {
	...
	RetADC = HAL_ADC_Start_DMA(&hadc1, buffer, 20);
	...
	while (1) {
		...
		#if defined(GRAPH) // For debug purposes
			extTrigger = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
			PWMOutput = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1);
			sprintf(bufferCDC, "%d\t%d\n", extTrigger, PWMOutput);
			CDC_Transmit_FS((uint8_t*) bufferCDC, strlen(bufferCDC));
		#elif defined(LCD)
			counts = HAL_ADC_GetValue(&hdma_adc1);
